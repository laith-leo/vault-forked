...
Vault now allows a single PKI mount to have multiple CA certificates
("issuers") in a single mount, for the purpose of facilitating rotation.
All issuers within a single mount are treated as a single authority,
meaning that CRL configuration and other authority URLs are common to
all issuers and serial numbers will be unique across all issuers. However,
since each issuer may have a distinct subject and keys, different issuers
may have different CRLs.
...
~> **Note**: Some functionality will not work if a default issuer is not
   configured. Vault automatically selects the default issuer as the
   present issuing certificate on migration from an older Vault version.
...
be read by the Vault CLI; use `/pki/cert/ca` or `pki/issuers` for that. If
`/pem` is added to the endpoint, the CA certificate is returned in PEM format.
...
~> **Note**: Vault now returns the full chain (including the default issuer's
   certificate and all parent issuers) in the `ca_chain` response, for both
   the `certificate` and `ca_chain` fields. The root certificate is no longer
   elided in the `certificate` field.
...
This endpoint retrieves the CRL _in raw DER-encoded form_. This is a
bare endpoint that does not return a standard Vault data structure and cannot
be read by the Vault CLI; use `/pki/cert/crl` for that. If `/pem` is added to
the endpoint, the CRL is returned in PEM format.
...
| `GET`  | `/pki/issuer/:issuer_ref/crl(/pem)` | selected  |
...
~> **Note**: Vault now returns the full chain (including this certificate and
   all parent issuers) in the `ca_chain` response. The root certificate is no
   longer elided.
...
via a PEM file containing the CA certificate and its private key, concatenated
together, in any order.

Each certificate will be validated to ensure it is a valid CA (has an asserted
basic constraint); non-CA certs will err. Any provided CRLs will be ignored.
Each unique certificate and private key will be imported as its own issuer or
key; duplicates (including with existing keys) will be ignored.
...
has expired or is close to expiring, administrators must hit this endpoint to
manually rotate the CRL.
